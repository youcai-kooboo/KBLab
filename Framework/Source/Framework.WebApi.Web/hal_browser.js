(function() {
  ...
  HAL.Router = Backbone.Router.extend({
    ...
    resourceRoute: function(url) {
      ...
      if (url.slice(0, 8) !== 'NON-GET:' && url.slice(0, 4) !== 'GET:') {
        this.client.get(url);
      }
    }
  });
  ...
  HAL.Views.QueryUriDialog = Backbone.View.extend({
    ...
    events: {
      ...
      'keyup .inputs': 'renderPreview',
      'change .inputs': 'renderPreview'
    },
    ...
    submitQuery: function(e) {
        ...
        var headersValue = this.headersValue();
        var method = 'GET';
        var body = '';
        
          var input;
          try {
            input = JSON.parse(this.$('.inputs').val());
          } catch(err) {
            input = {};
          }

          var uri = this.uriTemplate.expand(input);
          if(input) {
              for(key in input) {
                  var value = input[key];
                  if(!value) {
        ...
        var jqxhr = $.ajax({
            ...
            window.location.hash = 'GET:' + self.href;
            $('#request-headers').val(headersValue);
        });
    ...
    render: function() {
      ...
      this.$el.html(this.template({ href: this.href, user_defined_headers: $('#request-headers').val(), input: input }));
      this.$('.inputs').trigger('keyup');
  ...
  HAL.Views.NonSafeRequestDialog = Backbone.View.extend({
    ...
    render: function() {
        ...
        var input = this.createDefaultInput(this.extractExpressionNames(this.uriTemplate));
        var headerVal = $('#request-headers').val();
        if (headerVal.toLowerCase().indexOf('content-type:') < 0) {
            headerVal = 'content-type:application/json\t' + headerVal;
        }
        this.$el.html(this.template({ href: this.href, user_defined_headers: headerVal, input: input }));
        this.$('.inputs').trigger('keyup');
  ...
  // TemplateUrl Bugfixing
  HAL.buildTemplateUrl = function(href, data) {
    if(data) {
        for(var index in data) {
            var dataItem = data[index];
            var placeholder = '{' + dataItem.name + '}';
            if(href.indexOf(placeholder) == -1) {
                if(href.indexOf('?') == -1) {
                    href = href + '?' + dataItem.name + '=' + '{' + dataItem.name + '}';
                } else {
                    href = href + '&' + dataItem.name + '=' + '{' + dataItem.name + '}';
                }
            }
